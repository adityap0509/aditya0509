...
//Modal dikit napa
//Kasi credit MhankBar2 Maslent Xptn 
//dan juga Subscribe Aris187 ID jgn cuma ganti nama bot
...
ban = []
cr = '*ADITYA BOT*'
vz = '*Terima Kasih Sudah Daftar Di ADITYA BOT*'
const vcard = 'BEGIN:VCARD\n' //
            + 'VERSION:3.0\n' //
            + 'FN: A187ID\n' //
            + 'ORG:Creator ADITYA;\n' //
            + 'TEL;type=CELL;type=VOICE;waid=6285155446265:+62881024611545 \n' //
            + 'END:VCARD'
prefix = '!'
blocked = []
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' POWERED BY ADITYA'))
	})
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285155446266@s.whatsapp.net"]
			const mod = [ownerNumber,"6285155446265@s.whatsapp.net"]
			const adminbotnumber = ["6285155446265@s.whatsapp.net"]
			const frendsowner = ["6285155446265@s.whatsapp.net"]
			const premium = ["6285155446265@s.whatsapp.net"]
		...
		case 'iri':
         ...
         if (!isPremium) return reply(mess.only.premium)
          if (isBanned) return reply(mess.only.benned)   
                ...
                data = await fetchJson(`https://api.vhtear.com/ytmp3?query=${query}&apikey=Jsieu8287362jshre82`, {method: 'get'})
...
if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (love.length > 12) return reply('Teksnya kepanjangan, maksimal 9 karakter')
					reply(mess.wait)
					bufferxcz = await getBuffer(`https://api.vhtear.com/bannerff?title=${love}&text=MASLENT%20GANS&apikey=Jsieu8287362jshre82`, {method: 'get'})
					client.sendMessage(from, bufferxcz, image, {quoted: mek, caption: ' '+love})
					break
					case 'goldbutton':
				if (isBanned) return reply(mess.only.benned)    
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam wr.wb${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*pulsa :* _62881024611545_\n*gopay :* _6285155446265_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (isBanned) return reply(mess.only.benned)    
               ...
               if (!isUser) return reply(mess.only.userB)
                ...
                data = await fetchJson(`https://api.vhtear.com/music?query=${body.slice(6)}&apikey=Jsieu8287362jshre82`, {method: 'get'})
